...
export const loginServicio = async (correo, clavePlana) => {
    const conexion = await conexionBD();
    const [usuario, campos] = await conexion.execute('SELECT * FROM usuarios WHERE correo=?', [correo]);
    conexion.release();
...
export const crearUsuarioServicio = async(usuario)=>{
    ...
    const [resultado, campos] = await conexion.execute("INSERT INTO usuarios (nombre, apellido, correo, clave, rol) VALUES (?,?,?,?,?)", [usuario.nombre, usuario.apellido ,usuario.correo, clave, usuario.rol]);
    conexion.release();
    return resultado;
}
...
export const obtenerClientesServicio = async () => {
    const conexion = await conexionBD();
    const [clientes, campos] = await conexion.execute("SELECT id,nombre,apellido,correo FROM usuarios WHERE rol=?", ["Cliente"]);
    conexion.release();
    return clientes;
}